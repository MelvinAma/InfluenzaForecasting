...
class cola_gnn(nn.Module):  
    def __init__(self, args, data): 
        ...
        # MEx Thesis Adaptation - 2026-03-29
        # Purpose: Wire n_spatial to the hidsp argument so HPO can tune it.
        # The released code hard-coded n_spatial = 10; that value is now the
        # explicit default in runner.py and evaluate.py (hidsp=10).
        # Original:
        # self.n_spatial = 10
        self.n_spatial = args.hidsp
        # End Adaptation
    ...
    def forward(self, x, feat=None):
        ...
        # MEx Thesis Adaptation - 2026-03-10
        # Purpose: Batch all regions into a single Conv1d call instead of looping
        # over m regions individually. Produces identical outputs (Conv1d with
        # in_channels=1 processes each sample independently). ~20x faster.
        # Original:
        # r_l = []
        # r_long_l = []
        # h_mids = orig_x
        ...
        h_mids = orig_x
        h_all = h_mids.permute(0, 2, 1).contiguous().view(b * self.m, 1, w)
        r_l = self.conv(h_all).view(b, self.m, self.k, -1)
        r_long_l = self.conv_long(h_all).view(b, self.m, self.k, -1)
        # End Adaptation
