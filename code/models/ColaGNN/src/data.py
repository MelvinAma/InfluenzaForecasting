...
# MEx Thesis Adaptation - 2026-02-19
# Purpose: Bridge FluDataLoader to ColaGNN's DataBasicLoader interface,
#          enabling season-aware splits and configurable data paths.
# Original: DataBasicLoader loads from hardcoded ../data/ relative paths with ratio splits.
# Note: cola_gnn.__init__ recomputes adj internally from orig_adj via normalize_adj2,
#       so data.adj is not used by the model; only orig_adj needs to be correct.
import json as _json
import sys as _sys
from pathlib import Path as _Path
_sys.path.insert(0, str(_Path(__file__).resolve().parents[3] / "src" / "data"))
...
class MExDataLoader:
    def __init__(self, config_path, cuda=False):
        ...
        self.n = fl.n
        self.m = fl.m
        self.d = 0              # cola_gnn reads data.d; always 0 in original DataBasicLoader
        self.max = fl.max
        self.min = fl.min
        self.peak_thold = fl.peak_thold
        self.orig_adj = fl.orig_adj     # cola_gnn re-normalizes from orig_adj internally
        self.adj = fl.adj
        if cuda:
            self.orig_adj = self.orig_adj.cuda()
            self.adj = self.adj.cuda()
        ...
        self.val = fl.val
        self.test = fl.test
        self.train_set = fl.train_set
        self.val_set = fl.val_set
        self.test_set = fl.test_set
