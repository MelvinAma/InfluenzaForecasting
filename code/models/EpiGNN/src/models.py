...
class EpiGNN(nn.Module):
    ...
    def forward(self, x, index, isEval=False):
        ...
        if self.hw > 0:
            ...
            z = self.highway(z)
            z = z.view(-1, self.m)
            # MEx Thesis Adaptation - 2026-03-06
            # Purpose: Broadcast highway connection across quantile dimension
            # when output layer produces Q quantiles instead of 1 scalar.
            if res.dim() > z.dim():
                z = z.unsqueeze(-1)
            # End Adaptation
            res = res + z
