...
# Purpose: Bridge FluDataLoader to EpiGNN's DataBasicLoader interface,
#          enabling season-aware splits and configurable data paths.
# Original: DataBasicLoader loads from hardcoded relative paths with ratio splits.
...
class MExDataLoader:
    def __init__(self, config_path, cuda=False):
        ...
        self.adj = fl.adj           # raw adj; EpiGNN uses it directly (normalization commented out)
        self.orig_adj = fl.orig_adj
        self.degree_adj = fl.degree_adj
        if cuda:
            self.adj = self.adj.cuda()
            self.orig_adj = self.orig_adj.cuda()
            self.degree_adj = self.degree_adj.cuda()
