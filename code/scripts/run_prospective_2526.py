"""Prospective 2025/26 season evaluation.

Data was downloaded as a snapshot on 2026-03-05. Early 2025/26 weeks (Nov-Dec)
have likely been revised since FluSight participants submitted, so only the most
recent 2-3 weeks are genuinely preliminary. This gives us a smaller data vintage
advantage than 2024/25, but it is NOT a fully level playing field.

Vintaged re-evaluation shows -2.5% effect with preliminary data (negligible;
see data/vintages/vintage_comparison.json).

Horizon alignment same as 2024/25: our h=2,3,4 vs FluSight h=1,2,3.
"""
import json
import sys
...
from src.eval.flusight_hub import evaluate_hub_model, evaluate_all_hub_models
...
print(f"Seeds:  {SEEDS}")
print(f"Season: 2025/26 (PROSPECTIVE)")
print(f"Data:   snapshot from 2026-03-05. ~10/14 weeks use revised data; only 2-3 most recent weeks are genuinely preliminary.")
print()
...
for our_h, fs_h in ALIGNMENT.items():
    print(f"\n{'='*60}")
    print(f"COLAGNN h={our_h} (aligned with FluSight h={fs_h})")
    print(f"{'='*60}")
    ...
    P = 20
    min_idx = P + our_h - 1
    test_indices_h = [i for i in range(min_idx, len(seasons)) if seasons[i] == "2025/26"]
    test_dates_h = pd.DatetimeIndex([idx_to_date[i] for i in test_indices_h])
    print(f"Test dates: {test_dates_h[0].date()} to {test_dates_h[-1].date()} ({len(test_dates_h)} weeks)")
    ...
    print(f"Overlap weeks (with baseline): {len(overlap_tidx)}")

    if not overlap_tidx:
        print("  SKIP: no overlap weeks")
        continue

    overlap_dates = [test_dates_h[t] for t in overlap_tidx]
    print(f"Overlap range: {overlap_dates[0].date()} to {overlap_dates[-1].date()}")
    ...
    results_all[f"our_h{our_h}_vs_fs_h{fs_h}"] = {
        ...
        "overlap_start": str(overlap_dates[0].date()),
        "overlap_end": str(overlap_dates[-1].date()),
...
if all_qpred_combined:
    ...
    P = 20
    min_idx_2 = P + 2 - 1
    test_indices_ref = [i for i in range(min_idx_2, len(seasons)) if seasons[i] == "2025/26"]
    ...
    print(f"\n{'='*60}")
    print(f"COMBINED 2025/26 (our h=2,3,4 vs FluSight h=1,2,3)")
    print(f"{'='*60}")
    ...
    # Rank against all FluSight 2025/26 models
    print(f"\n{'='*60}")
    print("RANKING vs ALL FLUSIGHT 2025/26 MODELS (h=1,2,3 combined)")
    print(f"{'='*60}")

    ranking = evaluate_all_hub_models(
        hub_dir, test_dates_ref, gt_ref, STATES,
        horizons=[1, 2, 3],
    )
    print(ranking.to_string())

    ranking_path = ckpt_dir / "flusight_2526_ranking.csv"
    ranking.to_csv(ranking_path)

    our_rank = None
    for i, row in ranking.iterrows():
        if row["Rel. WIS"] >= combined_rel:
            our_rank = i
            break
    if our_rank is None:
        our_rank = len(ranking) + 1

    print(f"\nColaGNN would rank #{our_rank} with Rel. WIS = {combined_rel:.3f}")

    # Compare to 2024/25 results (load from canonical result file)
    print(f"\n{'='*60}")
    print("COMPARISON: 2024/25 vs 2025/26")
    print(f"{'='*60}")
    aligned_path = (
        PROJECT_ROOT / "results" / "native_quantile_v3" / "aligned_flusight_results.json"
    )
    if aligned_path.exists():
        with open(aligned_path) as f:
            prev = json.load(f)
        prev_rel = prev.get("combined_aligned", {}).get("rel_wis", "N/A")
        print(f"  2024/25 Rel. WIS: {prev_rel} (final/revised data)")
    print(f"  2025/26 Rel. WIS: {combined_rel:.3f} (snapshot data, NOT fully level playing field)")

out_path = ckpt_dir / "prospective_2526_results.json"
