"""FluSight comparison WITHOUT horizon alignment.

Compares our h=1,2,3 directly against FluSight h=1,2,3 (no +1 offset).
This gives our h=1 model a 1-week data advantage, producing implausibly
good results. The purpose is to demonstrate WHY horizon alignment matters.

The h=1 quantile checkpoints are trained by this script if they don't exist.
The h=2,3 checkpoints are reused from results/native_quantile_v3/.

Usage (run on Colab with A100):
    python scripts/run_unaligned_flusight.py
"""
import json
import sys
...
print(f"Seeds:  {SEEDS}")
print()
print("UNALIGNED: our h=1,2,3 directly vs FluSight h=1,2,3 (NO offset)")
print("This gives our h=1 a 1-week data advantage — expect implausibly good results.")

ALIGNMENT = {1: 1, 2: 2, 3: 3}

def cfg_path_for_h(h):
    if h == 1:
        return str(PROJECT_ROOT / "src" / "configs" / "us_colagnn.json")
    return str(PROJECT_ROOT / "src" / "configs" / f"us_colagnn_h{h}.json")
...
for our_h, fs_h in ALIGNMENT.items():
    print(f"\n{'='*60}")
    print(f"OUR h={our_h} vs FluSight h={fs_h} (UNALIGNED)")
    print(f"{'='*60}")

    c_path = cfg_path_for_h(our_h)
    P = 20
    min_idx = P + our_h - 1
    ...
    for seed in SEEDS:
        ...
        pred, true = inference_colagnn(
            ckpt["best_state"], cfg, c_path, split="test", device=DEVICE,
        )
...
P = 20
min_idx_1 = P + 1 - 1
test_indices_ref = [i for i in range(min_idx_1, len(seasons)) if seasons[i] == "2024/25"]
...
print(f"\n{'='*60}")
print(f"COMBINED UNALIGNED (our h=1,2,3 vs FluSight h=1,2,3)")
print(f"{'='*60}")
...
print()
print(f"For comparison, the ALIGNED result (our h=2,3,4 vs FS h=1,2,3) is 0.654.")
print(f"The difference ({combined_rel:.3f} vs 0.654) is entirely due to the")
print(f"1-week data advantage at h=1.")
...
out_path = ckpt_dir / "unaligned_flusight_results.json"
