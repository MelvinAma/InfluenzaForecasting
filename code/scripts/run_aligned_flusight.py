...
This script documents an earlier aligned-horizon ColaGNN FluSight rerun.
The thesis and the public release snapshot do NOT treat this script alone as
the authoritative benchmark path. The public benchmark authority is the stored
`results/vintage_matched_*` artifacts together with the shared scorer in
`src/eval/flusight_hub.py`.

FluSight participants submit forecasts on Wednesday with data available
through approximately reference_date - 7 days. This means:
  - FluSight h=1 (target = ref + 7d): effective gap from data = 14 days = 2 weeks
  - Our model h=1: input ends at target - 1 week = gap of 1 week

To align properly:
  - Our h=2 (gap = 2 weeks) matches FluSight h=1
  - Our h=3 (gap = 3 weeks) matches FluSight h=2
  - Our h=4 (gap = 4 weeks) matches FluSight h=3

This script trains h=2,3,4 models with native quantile loss and computes
relative WIS against FluSight models at the aligned horizons.

Note: this historical path still uses final/revised data values rather than
the preliminary data available at submission time, so it is not the
vintage-matched thesis benchmark.
"""
import json
import sys
...
print(f"Seeds:  {SEEDS}")
print()
print("HORIZON ALIGNMENT: our h=N+1 compares to FluSight h=N")
print("  Our h=2 -> FluSight h=1")
print("  Our h=3 -> FluSight h=2")
print("  Our h=4 -> FluSight h=3")

# Aligned mapping: our_horizon -> flusight_horizon
...
for our_h, fs_h in ALIGNMENT.items():
    print(f"\n{'='*60}")
    print(f"OUR h={our_h} (aligned with FluSight h={fs_h})")
    print(f"{'='*60}")

    cfg_path = str(PROJECT_ROOT / "src" / "configs" / f"us_colagnn_h{our_h}.json")
    ...
    # For overlap: use FluSight horizon fs_h to find reference dates
    # reference_date = target_end_date - fs_h * 7 days
    # Baseline file: {reference_date}-FluSight-baseline.csv
    overlap_tidx = []
    ...
    all_preds_h = []
    for seed in SEEDS:
        ckpt_path = ckpt_dir / f"colagnn_nq_h{our_h}_seed{seed}.pt"
        if ckpt_path.exists():
    ...
    # FluSight baseline WIS at the FLUSIGHT horizon
...
# Combined across aligned horizons
...
# Combined baseline WIS for FluSight h=1,2,3
P = 20
min_idx_2 = P + 2 - 1
...
print(f"\n{'='*60}")
print(f"COMBINED (aligned: our h=2,3,4 vs FluSight h=1,2,3)")
print(f"{'='*60}")
...
# Compare against FluSight models (values from hub ranking CSV, run-specific)
print(f"\n{'='*60}")
print("COMPARISON WITH FLUSIGHT MODELS (h=1,2,3 combined)")
print(f"{'='*60}")
print(f"  ColaGNN (aligned):         {combined_rel:.3f}")
...
if ranking_path.exists():
    ...
    for _, row in ranking_df.head(5).iterrows():
        print(f"  {row['Model']:<28s} {row['Rel. WIS']:.3f}")
else:
    print("  (Run evaluate_all_hub_models for full ranking)")

out_path = ckpt_dir / "aligned_flusight_results.json"
